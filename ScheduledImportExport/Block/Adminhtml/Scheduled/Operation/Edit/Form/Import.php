...
use Magento\ScheduledImportExport\Block\Adminhtml\Scheduled\Operation\Edit\Form\Import as CoreImport;
...
class Import extends CoreImport
{
    ...
    private function addSuccessEmailFieldset(
        ...
        $fieldset = $form->addFieldset(
            'import_success_emails',
            [
                'legend' => __('Import Success Emails'),
                'class'  => 'fieldset-wide'
            ]
        );

        $fieldset->addField('import_success_receiver', 'select', [
            'name'   => 'import_success[receiver]',
            'label'  => __('Success Email Receiver'),
            'title'  => __('Success Email Receiver'),
            'values' => $this->_emailIdentity->toOptionArray(),
        ]);

        $fieldset->addField('import_success_sender', 'select', [
            'name'   => 'import_success[sender]',
            'label'  => __('Success Email Sender'),
            'title'  => __('Success Email Sender'),
            'values' => $this->_emailIdentity->toOptionArray(),
        ]);

        $fieldset->addField('import_success_template', 'select', [
            'name'   => 'import_success[template]',
            'label'  => __('Success Email Template'),
            'title'  => __('Success Email Template'),
            'values' => $this->getSuccessTemplates(),
        ]);

        $fieldset->addField('import_success_copy_to', 'textarea', [
            'name'  => 'import_success[copy_to]',
        ...
        $fieldset->addField('import_success_copy_method', 'select', [
            'name'   => 'import_success[copy_method]',
        ...
        $form->addValues([
            'import_success_receiver'    => (string)$successConfig['receiver'],
            'import_success_sender'      => (string)$successConfig['sender'],
            'import_success_template'    => (string)$successConfig['template'],
            'import_success_copy_to'     => (string)$copyTo,
            'import_success_copy_method' => (string)$successConfig['copy_method'],
        ]);
    }
    ...
    private function getSuccessTemplates(): array
    {
        $templates = [
            [
                'value' => 'magento_scheduledimportexport_import_success',
                'label' => __('Import Success (Default)')
            ]
        ];
        ...
        try {
            ...
            foreach ($allTemplates as $template) {
                if (
                    ...
                    $template['value'] === 'magento_scheduledimportexport_import_success'
                ) {
                    continue;
                }
